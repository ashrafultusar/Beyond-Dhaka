...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${isWhiteBg ? "bg-white shadow-md border-b border-gray-100" : "bg-transparent"
        }`}
    >
      <div className="container flex items-center justify-between h-20 md:h-24 px-6 mx-auto">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                href={item.to}
                className={`px-5 py-2 text-sm font-semibold transition-all rounded-full ${isActive ? "text-[#0d6335] bg-[#f96f1f]/10" : "text-gray-600 hover:text-[#0d6335]"
                  }`}
              >
                {item.label}
              </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Mobile Menu Button */}
          <button
            className={`lg:hidden p-2 rounded-lg transition-colors ${isWhiteBg ? "text-[#0d6335] hover:bg-gray-100" : "text-[#0d6335] bg-white/80 hover:bg-white"
              }`}
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[300px] bg-white z-[70] shadow-2xl transform transition-transform duration-500 ease-in-out ${open ? "translate-x-0" : "translate-x-full"
          }`}
      >
        ...
        <div className="p-6 flex flex-col gap-2">
          {navItems.map((item) => (
            <Link
              key={item.to}
              href={item.to}
              className={`block p-4 rounded-xl font-semibold transition-colors ${pathname === item.to
